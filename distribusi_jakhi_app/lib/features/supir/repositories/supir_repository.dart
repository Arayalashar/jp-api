...
class SupirRepository {
  ...
  Future<List<dynamic>> fetchRiwayat(String idSupir) async {
    final response = await ApiClient.get('${ApiConfig.riwayatSupir}?id_supir=$idSupir');
    
    if (response['status'] == 'success') {
      return response['data'] ?? [];
    } else {
      throw Exception(response['message'] ?? 'Gagal memuat riwayat supir');
    }
  }
