...
class GudangRepository {
  ...
  Future<List<GudangModel>> fetchRiwayat(String idGudang) async {
    final response = await ApiClient.get('${ApiConfig.riwayatGudang}?id_karyawan=$idGudang');
    
    if (response['status'] == 'success') {
      final List data = response['data'] ?? [];
      return data.map((e) => GudangModel.fromJson(e)).toList();
    } else {
      throw Exception(response['message'] ?? 'Gagal memuat riwayat packing');
    }
  }
