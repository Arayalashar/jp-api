...
if(isset($data->id_dokumen) && isset($data->id_karyawan)) {
    ...
    if($update_success && $log_success) {
        // Cari id_supir dari dokumen ini
        $stmt_get_supir = $koneksi->prepare("SELECT id_supir, nomor_dokumen FROM dokumen WHERE id_dokumen = ?");
        if($stmt_get_supir) {
            $stmt_get_supir->bind_param("i", $id_dokumen);
            $stmt_get_supir->execute();
            $res_supir = $stmt_get_supir->get_result();
            if($row_supir = $res_supir->fetch_assoc()) {
                buat_notifikasi_user($koneksi, $row_supir['id_supir'], 'Barang Siap Dikirim', "Dokumen {$row_supir['nomor_dokumen']} sudah dipacking dan siap dikirim.");
            }
            $stmt_get_supir->close();
        }
